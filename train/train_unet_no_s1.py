"""
Training script for the UNet denoiser with S2-ONLY conditioning (no S1).

This is the ablation counterpart of train_unet.py: identical in every respect
except the conditioning drops the 2 Sentinel-1 SAR channels, so the UNet sees
only the optical S2 latent. Train both and compare val_loss / downstream metrics
to measure whether passing S1 actually helps.

Differences vs train_unet.py:
    - conditioning = VAE(S2) [4ch]            (train_unet.py: concat(VAE(S2)[4ch], S1[2ch]) = 6ch)
    - UNet in_channels = 8                     (train_unet.py: 10)
    - default config   = config_10m_no_s1.yaml (train_unet.py: config_10m.yaml)
    - default ckpt dir = checkpoints/unet_no_s1
The dataset / split / seed are reused unchanged, so the two runs see identical
train/val/test tiles — the only variable is the presence of S1.

Usage:
    python train_unet_no_s1.py --data_dir /path/to/npz_tiles --vae_ckpt /path/to/vae.ckpt
    python train_unet_no_s1.py --data_dir data --vae_ckpt checkpoints/vae.ckpt --epochs 50 --batch_size 4
...
Training flow per step (1m aerial, scale_factor=8, padded 128→1024):
    ...
    2. S2 (B,4,128,128) → normalize → upsample 1024 → VAE encode → cond_s2 (B,4,128,128)
       conditioning = cond_s2 → (B,4,128,128)   [no S1]
...
from opensr_model.utils import normalize_s2, normalize_aerial

VAE_SCALE_FACTOR = 0.18215


# ──────────────────────────────────────────────────────────────────────────────
# Pre-computed latent dataset (S1-free)
# ──────────────────────────────────────────────────────────────────────────────

class LatentFusionDatasetNoS1(LatentFusionDataset):
    """Pre-computed-latent dataset for the S2-only model.

    Subclasses LatentFusionDataset so tile membership and ordering are *identical*
    to the S1+S2 model (same rglob + same _tile_ok filter ⇒ same random_split ⇒
    fair ablation). The only change is __getitem__: it loads just the latent .pt
    and never touches the NPZ, so the per-step S1 load is skipped entirely.

    Note: _tile_ok (inherited) still requires the S1 keys to be present in each
    NPZ. That is intentional — it keeps the accepted tile set byte-identical to
    the S1 run, even though S1 itself is never read here.
    """
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if "aerial_mean" in lat:
            ...
            z_aerial = (lat["aerial_mean"].float()
                        + torch.exp(0.5 * lat["aerial_logvar"].float())
                        * torch.randn(lat["aerial_mean"].shape)) * VAE_SCALE_FACTOR
            z_s2 = (lat["s2_mean"].float()
                    + torch.exp(0.5 * lat["s2_logvar"].float())
                    * torch.randn(lat["s2_mean"].shape))
        else:
            # Backward compat: old files store precomputed z directly
            z_aerial = lat["z_aerial"]
            z_s2     = lat["z_s2"]

        return {"z_aerial": z_aerial, "z_s2": z_s2, "path": str(path)}
...
class LitUNetDenoiser(pl.LightningModule):
    """PyTorch Lightning module for training the UNet denoiser (S2-only).

    Wraps a LatentDiffusion model with frozen VAE and trainable UNet.
    Conditioning is built on-the-fly from S2 only — no S1.
    """
    ...
    def _build_conditioning(self, s2):
        """Build 4-channel conditioning: VAE(S2) only (no S1)."""
        lr_size = s2.shape[-1]
    ...
    def _shared_step(self, batch):
        ...
        if "z_aerial" in batch:
            # Pre-computed path — no VAE VRAM. S1 (s1_cond) is ignored.
            z_0          = batch["z_aerial"].float().to(self.device)  # (B, 4, 128, 128)
            conditioning = batch["z_s2"].float().to(self.device)      # (B, 4, 128, 128)
            ...
            # On-the-fly path — original behaviour, S1 ignored
            z_0          = self._encode_aerial(batch["aerial"])
            conditioning = self._build_conditioning(batch["s2"])
...
class AugmentedDataset(Dataset):
    """Wraps a dataset split and applies random flips + 90° rotations.

    The same transform is applied identically to the S2 conditioning and the
    aerial target so spatial alignment is preserved. S1 is not augmented (it is
    never read by the S2-only model). Training split only.
    """
...
class AugmentedDatasetX8(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        keys = ["z_aerial","z_s2"] if "z_aerial" in sample else ["s2","aerial"]
...
class FusionDataModule(pl.LightningDataModule):
    ...
    using torch.utils.data.random_split (deterministic seed). Identical to
    train_unet.py so the no-S1 run sees the exact same split.
    """
    ...
    def setup(self, stage=None):
        if self.train_ds is None:
            if self.latent_dir:
                full_ds = LatentFusionDatasetNoS1(root=self.data_dir, latent_dir=self.latent_dir)
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Train UNet denoiser with S2-only conditioning (no S1)")
    ...
    parser.add_argument("--config", type=str, default=None, help="Path to YAML config (default: config_10m_no_s1.yaml)")
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/unet_no_s1", help="Directory to save model checkpoints")
    ...
    if args.config is None:
        config_path = pathlib.Path(__file__).resolve().parent.parent / "opensr_model" / "configs" / "config_10m_no_s1.yaml"
    else:
    ...
    callbacks = [
        ModelCheckpoint(
            ...
            filename="unet-no-s1-{epoch:04d}-{val_loss:.6f}",
            monitor="val_loss",
            mode="min",
            save_top_k=3,
            save_last=True,
    ...
    # ── Logger ──────────────────────────────────────────────────────────────
    logger = TensorBoardLogger(save_dir=args.log_dir, name="unet_s2_only")
