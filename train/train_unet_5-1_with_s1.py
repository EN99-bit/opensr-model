"""
Training script for a 5m→1m UNet denoiser (cascade stage 2).

This is stage 2 of a two-stage super-resolution cascade:
...
def main():
    parser = argparse.ArgumentParser(description="Train 5m→1m UNet denoiser (cascade stage 2)")
    ...
    callbacks = [
        ModelCheckpoint(
            dirpath="checkpoints/5to1m/unet",
            filename="unet5to1-{epoch:04d}-{val_loss:.6f}",
            monitor="val_loss",
            mode="min",
            save_top_k=3,
            save_last=True,
    ...
    logger = TensorBoardLogger(save_dir=args.log_dir, name="unet_5to1m")
    ...
    if args.find_lr:
        ...
        fig.savefig("lr_finder_5to1m.png")
        print("LR finder plot saved to: lr_finder_5to1m.png")
        return
